...
array_dtl_check(void* array)
{
    ...
    if (array)
    {
        ARRAY_ASSERT(array_dtl_get_capacity(array) >= array_dtl_get_size(array));
    }
}
...
ARRAY_FUNC_DECORATOR int array_dtl_reserve(void** array_ptr, ptrdiff_t dest_size,
                                           ptrdiff_t item_size);
...
ARRAY_FUNC_DECORATOR int array_dtl_fit(void** array_ptr, ptrdiff_t item_size);


#define array_size(ARRAY)                                                      \
    (array_dtl_check(ARRAY), array_dtl_get_size(ARRAY))

#define array_capacity(ARRAY)                                                  \
    (array_dtl_check(ARRAY), array_dtl_get_capacity(ARRAY))

#define array_isempty(ARRAY)                                                   \
    (array_dtl_check(ARRAY), array_size(ARRAY) == 0)
...
#define array_clear(ARRAY)                                                     \
    (array_dtl_check(ARRAY),                                                   \
     ((ARRAY)                                                                  \
      ? array_dtl_clear(array_dtl_get_header(ARRAY))                           \
      : (void)0))
...
#define array_popfit(ARRAY)                                                    \
    ((void)(array_pop(ARRAY)), array_fit(ARRAY))

#define array_popnfit(ARRAY, N)                                                \
    ((void)(array_popn(ARRAY, N)), array_fit(ARRAY))

#define ARRAY_INLINE_STORAGE(N, TYPE)                                          \
    (sizeof(array_header) + ((((N) + 1) & (~1)) * sizeof(TYPE)))

#define array_inline(BUFFER, N)                                                \
    array_dtl_inline_init((BUFFER), (N))
...
array_dtl_fit(void** array_ptr, ptrdiff_t item_size)
{
    ...
    {
        size_t alloc_size = sizeof(array_header) + (size_t)(new_capacity * item_size);
...
ARRAY_FUNC_DECORATOR inline array_header* array_dtl_get_header(void* array);

ARRAY_FUNC_DECORATOR inline ptrdiff_t array_dtl_get_size(void* array);

ARRAY_FUNC_DECORATOR inline ptrdiff_t array_dtl_get_capacity(void* array);

ARRAY_FUNC_DECORATOR inline int array_dtl_need_resize(void* array, ptrdiff_t n);

ARRAY_FUNC_DECORATOR inline int array_dtl_is_inline(void* array);

ARRAY_FUNC_DECORATOR inline void array_dtl_check(void* array);

ARRAY_FUNC_DECORATOR inline ptrdiff_t array_dtl_get_size_and_incr(void* array, ptrdiff_t n);

ARRAY_FUNC_DECORATOR inline ptrdiff_t array_dtl_decr_and_get_size(void* array, ptrdiff_t n);

ARRAY_FUNC_DECORATOR inline void* array_dtl_inline_init(void* mem, ptrdiff_t capacity);
